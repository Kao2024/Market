...
public class ServiceMenu implements ActionListener{
	...
	private JPanel getService1JPanel() {
		...
		JLabel label = new JLabel("Enter the light number")	;
		panel.add(label);
		...
		JButton buttonOn = new JButton("Turn On");
		buttonOn.addActionListener(this);
		panel.add(buttonOn);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
                
                JButton buttonOff = new JButton("Turn Off");
	...
	private JPanel getService2JPanel() {
		...
		JLabel label = new JLabel("Enter the product number for inventory check")	;
		panel.add(label);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
		...
		JButton button = new JButton("Stock");
	...
	private JPanel getService3JPanel() {
		...
		JLabel label = new JLabel("Enter the product number for standard price check")	;
		panel.add(label);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
		...
		JButton button = new JButton("Unit price");
		button.addActionListener(this);
		panel.add(button);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
	...
	private JPanel getService4JPanel() {
		...
		JLabel label = new JLabel("Customize order")	;
		panel.add(label);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
		...
		JButton button = new JButton("Estimated price");
		button.addActionListener(this);
		panel.add(button);
		panel.add(Box.createRigidArea(new Dimension(10, 0)));
	...
	public static void main(String[] args) {
		...
		ServiceMenu gui = new ServiceMenu();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (label.equals("Turn on")) {
			...
			generated.ds.service1.LightRequest request= generated.ds.service1.RequestMessage.newBuilder().setText(entry1.getText()).build();

			//retrieving reply from service
			generated.ds.service1.LightResponse response= blockingStub.TurnOn(request);

			reply1.setText( String.valueOf( response.getLength()) );
			channel.shutdown();
		
		}else if (label.equals("Turn off")) {
			System.out.println("service 1 to be invoked ...");
			...
			//retrieving reply from service
			generated.ds.service1.ResponseMessage response = blockingStub.TurnOff(request);

			reply1.setText(response.getLightNumber() + " : " + response.getPrice() );
			channel.shutdown();
		
		}else if (label.equals("Stock")) {
			...
			//preparing message to send
			generated.ds.service2.RequestMessage request = generated.ds.service2.RequestMessage.newBuilder().setProductNumber(entry2.getText()).build();

			//retrieving reply from service 
			generated.ds.service2.ResponseMessage response =  blockingStub.service2Do(request);
			reply2.setText(response.getProductNumber() + " : " + response.getStock());
			channel.shutdown();
			 
	     
			
		}else if (label.equals("Unit price")) {
			...
			//preparing message to send
			generated.ds.service3.RequestMessage request = generated.ds.service3.RequestMessage.newBuilder().setProductNumber(entry3.getText()).build();

			//retreving reply from service
			generated.ds.service3.ResponseMessage response =  blockingStub.service3Do(request);
			reply3.setText(response.getProductNumber() + " : " + response.getPrice());
			channel.shutdown();
                        
		
		}else if (label.equals("Estimated price")) {
